...
#'   for fill color. Default: `"total_overlap"`.
#' @param border_color Character string. Polygon border color. Default: `"grey40"`.
#' @param na_color Character string. Fill color for missing values. Default: `"white"`.
...
#'
#' @param track_data A data frame or tibble containing longitude (`lon`) and latitude (`lat`) columns.
...
#' @param trip_ids Character or integer vector of trip IDs to plot. If \code{NULL} (default), all trips are shown.
#' @param colony_coords Named numeric vector with \code{lon} and \code{lat}.
#' @param color_by Character. Column to color-code trips by. Default: \code{"trip_id"}.
#' @param title Character. Plot title.
#'
...
#' most time across the study area.
#'
