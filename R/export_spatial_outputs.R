...
#'       GPS tracks.}
#'     \item{\code{core_area}}{An \code{sf} POLYGON object (50 percent UD contour).}
#'     \item{\code{home_range}}{An \code{sf} POLYGON object (95 percent UD contour).}
...
export_spatial_outputs <- function(results,
  ...
  .resolve_path <- function(name, ext) {
    ...
    if (file.exists(p) && !overwrite) {
      warning(
        "File already exists and overwrite = FALSE; skipping: ", p,
        call. = FALSE
      )
      return(NULL)
    }
  ...
  for (key in intersect(gis_keys, names(results))) {
    ...
    if (!inherits(layer, c("sf", "sfc", "RasterLayer", "SpatRaster"))) {
      warning(
        "`results$", key, "` is not a recognised spatial object; skipping.",
        call. = FALSE
      )
      next
    }
  ...
  # UD polygons via dedicated exporter if core/home-range are present
  if (all(c("core_area", "home_range") %in% names(results))) {
    ...
    if (!is.null(p)) {
      .msg(
        "[export_spatial_outputs] Writing combined UD contours -> ",
        basename(p)
      )
  ...
  for (key in intersect(table_keys, names(results))) {
    ...
    if (!is.data.frame(tbl)) {
      warning(
        "`results$", key, "` is not a data frame; skipping.",
        call. = FALSE
      )
      next
    }
